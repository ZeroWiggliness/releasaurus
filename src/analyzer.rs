...
impl Analyzer {
    ...
    fn process_first_release(&self, release: &mut Release) -> Result<()> {
        ...
        if let Some(ref prerelease_id) = self.config.prerelease {
            semver = helpers::add_prerelease(semver, prerelease_id, self.config.prerelease_version)?;
        }
    ...
    fn calculate_next_prerelease_version(
        ...
        if current.semver.pre.is_empty() {
            ...
            let next_stable =
                version_updater.increment(&current.semver, commits);
            let version = helpers::add_prerelease(next_stable, prerelease_id, self.config.prerelease_version)?;
            Ok(version)
        } else {
            ...
            if current_pre_id == prerelease_id {
                ...
                let version =
                    helpers::add_prerelease(stable_next, prerelease_id, self.config.prerelease_version)?;
                Ok(version)
            }
        }
    }
